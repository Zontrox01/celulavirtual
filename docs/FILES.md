...
El directorio `gui/` no estaba previsto en la arquitectura original del whitepaper (sección 5.4 no contemplaba una interfaz gráfica de escritorio). Se añadió con la decisión de diseño de mantener la lógica de negocio (`gui/app_state.py`, `gui/breakpoint_builder.py`) completamente separada de Qt, para poder testearla igual que el resto del proyecto.

**Cambio de librería a mitad de desarrollo**: se empezó a construir con `PyQt6`, pero se corrigió a **`PySide6`** antes de entregar ningún archivo. Ambas son bindings de Qt6 y comparten casi toda la API (incluidos los enums con ámbito, como `QPalette.ColorRole.Window`); las únicas diferencias aplicadas fueron el nombre del paquete (`PyQt6` → `PySide6`) y el nombre de las señales (`pyqtSignal` → `Signal`).
